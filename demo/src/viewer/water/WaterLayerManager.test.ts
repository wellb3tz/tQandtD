...
describe('WaterLayerManager - Integration Tests', () => {
  ...
  describe('applyLOD - Ocean Only', () => {
    it('should control only ocean visibility', () => {
      // **Validates: Requirement 4.7**
      const chunkData = createMockChunkWithOcean();
      const configWithLOD = {
      ...
      manager.addWaterToChunk('0,0', chunkData, scene, configWithLOD);
      
      const waterLayer = manager.getWaterLayer('0,0');
      const cameraPosition = new THREE.Vector3(0, 0, 0);
      
      // Apply LOD at near distance
      manager.applyLOD(cameraPosition, configWithLOD);
      
      // Verify ocean meshes are visible
      expect(waterLayer!.group.visible).toBe(true);
      for (const waterMesh of waterLayer!.ocean) {
        expect(waterMesh.mesh.visible).toBe(true);
      }
    });

    it('should show water at near distance', () => {
      const chunkData = createMockChunkWithOcean();
      const configWithLOD = {
        ...DEFAULT_WATER_CONFIG,
        performance: {
      ...
      // Test near distance (< 200 units) - should be visible
      const cameraPosition = new THREE.Vector3(100, 0, 0);
      manager.applyLOD(cameraPosition, configWithLOD);
      expect(waterLayer!.group.visible).toBe(true);
    });

    it('should hide water at far distance', () => {
      const chunkData = createMockChunkWithOcean();
      const configWithLOD = {
        ...DEFAULT_WATER_CONFIG,
        performance: {
      ...
      // Test far distance (>= 500 units) - should be hidden
      const cameraPosition = new THREE.Vector3(1000, 0, 0);
      manager.applyLOD(cameraPosition, configWithLOD);
      expect(waterLayer!.group.visible).toBe(false);
    });

    it('should not apply LOD when disabled in config', () => {
      const chunkData = createMockChunkWithOcean();
      const config = {
        ...DEFAULT_WATER_CONFIG,
        performance: {
      ...
      const cameraPosition = new THREE.Vector3(1000, 0, 0);
      manager.applyLOD(cameraPosition, config);
      
      // Visibility should not change when LOD is disabled
      expect(waterLayer!.group.visible).toBe(true);
    });

    it('should keep ocean visible at medium distance', () => {
      const chunkData = createMockChunkWithOcean();
      const configWithLOD = {
        ...DEFAULT_WATER_CONFIG,
        performance: {
      ...
      const waterLayer = manager.getWaterLayer('0,0');
      const cameraPosition = new THREE.Vector3(300, 0, 0); // Medium distance
      
      manager.applyLOD(cameraPosition, configWithLOD);
      
      // Verify water group is visible
      expect(waterLayer!.group.visible).toBe(true);
