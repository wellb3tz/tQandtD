...
**Performance:**
- Without workers: 16 x 20ms = 320ms
- With 4 workers: ~80ms (4x faster)
...
interface WorkerPoolConfig {
  ...
  createWorker?: (url: string) => Worker; // Custom worker factory
}
```
...
// Save only chunks with recorded modifications
...
async function profileGeneration(
  ...
  console.log(`Rivers: ${chunk.rivers?.length || 0}`);
}
```
...
function logCacheStats(manager: ChunkManager): void {
  ...
  console.log(`  Memory: ${(stats.size * 6.2).toFixed(1)} KB`);
}
