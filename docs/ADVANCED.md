...
Track changes to chunks for delta saves:
...
Modify generated terrain at runtime.

### Height Editing

```typescript
import { localToIndex } from 'procedural-world-engine';
...
function setHeight(
  ...
  const index = localY * vertexSize + localX;
  chunk.heightmap[index] = height;
  
  // Record modification
  manager.recordTerrainEdit(chunk.x, chunk.y, index, height);
}

// Raise terrain at (5, 10)
const chunk = await manager.getChunk(0, 0);
setHeight(chunk, 5, 10, 0.8);
```
    ...
    for (let y = 0; y < vertexSize; y++) {
      for (let x = 0; x < vertexSize; x++) {
        ...
        if (distance <= radius) {
          ...
          const newHeight = Math.min(1, currentHeight + strength * falloff);
          chunk.heightmap[index] = newHeight;
          
          manager.recordTerrainEdit(chunk.x, chunk.y, index, newHeight);
        }
      }
    }
  }
    ...
    for (let y = 0; y < vertexSize; y++) {
      for (let x = 0; x < vertexSize; x++) {
        ...
        if (distance <= radius) {
          ...
          const index = y * vertexSize + x;
          const currentHeight = chunk.heightmap[index];
          const newHeight = currentHeight + (targetHeight - currentHeight) * falloff;
          chunk.heightmap[index] = newHeight;
        ...
        if (distance <= radius) {
          ...
          for (let ny = -1; ny <= 1; ny++) {
            for (let nx = -1; nx <= 1; nx++) {
              ...
              if (sx >= 0 && sx < vertexSize && sy >= 0 && sy < vertexSize) {
                const sIndex = sy * vertexSize + sx;
                sum += chunk.heightmap[sIndex];
                count++;
              }
            }
          }
          ...
          edits.set(index, newHeight);
        }
      }
    }
    
    // Apply edits
    for (const [index, height] of edits) {
      chunk.heightmap[index] = height;
    }
    
    manager.recordTerrainEdits(chunk.x, chunk.y, edits);
  }
}
```
...
function addStructure(
  ...
  const structure = { x, y, type };
  chunk.structures.push(structure);
  
  manager.recordStructureAddition(chunk.x, chunk.y, structure);
}
...
function removeStructure(
  ...
  chunk.structures.splice(index, 1);
  
  manager.recordStructureRemoval(chunk.x, chunk.y, index);
}
```
