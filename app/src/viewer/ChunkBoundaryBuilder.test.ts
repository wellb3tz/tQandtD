...
describe('ChunkBoundaryBuilder', () => {
  it('draws only the top and left chunk edges in world coordinates', () => {
    ...
    expect(position.getX(0)).toBe(4 * TERRAIN_TILE_SIZE_METERS);
    expect(position.getY(0)).toBeCloseTo(0.1 * TERRAIN_HEIGHT_SCALE_METERS);
    expect(position.getZ(0)).toBe(6 * TERRAIN_TILE_SIZE_METERS);
    expect(position.getX(1)).toBe(6 * TERRAIN_TILE_SIZE_METERS);
    expect(position.getY(1)).toBeCloseTo(0.3 * TERRAIN_HEIGHT_SCALE_METERS);
    expect(position.getZ(1)).toBe(6 * TERRAIN_TILE_SIZE_METERS);
    expect(position.getX(3)).toBe(4 * TERRAIN_TILE_SIZE_METERS);
    expect(position.getY(3)).toBeCloseTo(0.6 * TERRAIN_HEIGHT_SCALE_METERS);
    expect(position.getZ(3)).toBe(8 * TERRAIN_TILE_SIZE_METERS);
  });
  ...
  it('uses seamless heightmap indexing for the bottom-left corner', () => {
    ...
    expect(position.getY(3)).toBeCloseTo(0.6 * TERRAIN_HEIGHT_SCALE_METERS);
    expect(position.getY(3)).not.toBeCloseTo(0.5 * TERRAIN_HEIGHT_SCALE_METERS);
  });
});
